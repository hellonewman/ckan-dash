...
org_id <- "9ecaff80-fb4a-457b-8141-e53f7c991890"

# Get the users for the organization
users <- reactive({
  data <- fromJSON(paste0("https://data.wprdc.org/api/3/action/organization_show?id=", org_id))$result
  users <- data$users
})

# Grab all datasets on WPRDC and filter them by org id
dataSets <- reactive({
  data <- fromJSON("https://data.wprdc.org/api/action/package_search?rows=1000&start=0")$result
  dataSets <- select(data$results, -c(relationships_as_object, relationships_as_subject, extras, groups)) %>%
    ...
    mutate(metadata_modified = as.POSIXct(metadata_modified, format = "%Y-%m-%dT%H:%M:%S"),
           ...
           # Resource link
           title = paste0('<a href="https://data.wprdc.org/dataset/', name, '" target="_blank">', title, '</a>'),
...
DT::renderDataTable({
  userTable <- users() %>%
    select(display_name, name, capacity, state, created, number_created_packages) %>%
